...
and glean can extract that data.
...
schedule says is due, which settles at a few thousand lookups a day -- see [Why
the second scan is cheap](#why-the-second-scan-is-cheap).
...
What the default set actually tolerates:
...
The file can become a full database in the future, if required.

The word lists are configuration, not data, and live in the XDG *config*
directory instead -- `~/.config/glean/sources.d/`.
...
`/usr/share/dict/words`, because that path is not the same file everywhere.
